...
class GATConv(nn.Module):
    ...
    def forward(self, graph, feat, edge_weight=None, get_attention=False):
        ...
        with graph.local_scope():
            ...
            self.output_for_debug["feat_src"] = feat_src
            self.output_for_debug["feat_dst"] = feat_dst
            ...
            self.output_for_debug["el"] = el
            self.output_for_debug["er"] = er

            # compute edge attention, el and er are a_l Wh_i and a_r Wh_j respectively.
            graph.apply_edges(fn.u_add_v("el", "er", "e"))
            e = self.leaky_relu(graph.edata.pop("e"))

            self.output_for_debug["e"] = e

            self.output_for_debug["a_after_softmax"] = edge_softmax(graph, e)

            # compute softmax
            graph.edata["a"] = self.attn_drop(edge_softmax(graph, e))

            self.output_for_debug["a_before"] = graph.edata["a"]
            ...
            if edge_weight is not None:
                ...
                self.output_for_debug["edge_weight"] = edge_weight.tile(
                    1, self._num_heads, 1
                ).transpose(0, 2)

            self.output_for_debug["a"] = graph.edata["a"]

            # message passing
            graph.update_all(fn.u_mul_e("ft", "a", "m"), fn.sum("m", "ft"))
            rst = graph.dstdata["ft"]

            self.output_for_debug["rst"] = rst
            # residual
            if self.res_fc is not None:
                ...
                self.output_for_debug["resval"] = resval
            ...
            if self.has_explicit_bias:
                ...
                self.output_for_debug["rst_after_bias"] = rst
            # activation
            if self.activation:
...
class GATLayer(nn.Module):
    ...
    def forward(self, h, g, edge_weight=None, get_attention=False):
        ...
        self.output_for_debug["feat_src"] = Wh_src
        self.output_for_debug["feat_dst"] = Wh_dst

        # Compute attention scores
        e_l = (Wh_src * self.attn_l).sum(dim=-1)  # Shape (N, 1, num_heads)
        e_r = (Wh_dst * self.attn_r).sum(dim=-1)  # Shape (1, N, num_heads)

        self.output_for_debug["e_l"] = e_l
        self.output_for_debug["e_r"] = e_r
        ...
        self.output_for_debug["a_after_softmax"] = attention

        attention = self.attn_drop(attention)

        self.output_for_debug["a_before"] = attention
        ...
        self.output_for_debug["a"] = attention
        ...
        self.output_for_debug["rst"] = h_prime
        ...
        if self.bias is not None:
            ...
            self.output_for_debug["rst_after_bias"] = h_prime
...
def debug_gat_layer_homogeneous_with_edge_weights():
    import dgl
